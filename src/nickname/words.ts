...
//
// 総当たりにすると「もっちもちの香車」「ほかほかのかき氷」のような噛み合わない名前が出るので、
// 語には**札**を持たせ、札が重なる組み合わせだけを作る。札は2階建て:
...
//   - **MODS に「種類」と「その下位の札」を一緒に付けない。** 札は1つでも重なれば通るので、
//     たとえば `["ほかほか", FOOD | WARM]` と書くと FOOD だけで全部の食べものが通り、
//     WARM の絞り込みが消える（「ほかほかのかき氷」が復活する）。付ける先を広げたいときは、
//     NOUNS 側に下位の札を足すほうで直す（例:「ふわふわのざぶとん」を作りたいなら ざぶとん に AIRY を足す）。
//   test/nickname.spec.ts が上を機械的に見張る。
//   語を消したり札を狭めたりしてもよい（作れなくなった名前を持っている再訪者は、
//   online-match.js が読み込み時に照合して黙って引き直す）。
...
// 語の札を引く。無い語は 0（＝どれとも重ならない）。
// Map を作らず素の走査にしているのは、ブラウザ側へ連結するぶんを小さくするため
// （語は100語ほどで、呼ぶのは名前を作るときと検証するときだけ）
function kindOf(list: readonly Word[], word: string): number {
  for (const [w, kind] of list) {
    if (w === word) return kind;
  }
  return 0;
}
 ...
 *  1つ1つの名前が出やすくなり、「さくさくのせんべい」が46人に1人になってしまう。
 *  当たりは 1,317/2,392（55%）なので平均1.8回で決まる */
export function randomName(): string {
  ...
  // 50回外すのは 1京分の1以下（組み合わせが1つも無い語彙はテストで落ちる）。念のための出口
  const [mod, mask] = MODS[0];
  return `${mod}の${NOUNS.find(([, kind]) => kind & mask)![0]}`;
}

/** いまの語彙から作れる名前か。サーバーが日本語の表示名を通す唯一の条件で、
 *  ブラウザ側は保存済みの名前がまだ作れるか（語彙を直していないか）の確認に使う */
export function isGeneratedName(name: string): boolean {
  const at = name.indexOf("の");
  if (at <= 0) return false;
  return (kindOf(MODS, name.slice(0, at)) & kindOf(NOUNS, name.slice(at + 1))) !== 0;
}
