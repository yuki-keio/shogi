...
**命名規則**: 内部の識別子に**ランダム前提の語（`quick` / `random` / `casual`）を使わない**。将来「実力値の近い2人から組む」方式に組み直す可能性があるので、組み合わせ方が変わっても意味が変わらない**機構ベースの名前**にする。
...
| **作らない** | チャット／通報・ブロック・BAN／新しいページ（すべて `/online/` 内で完結） |

> 🔴 **2026-08-25 追記**: この表にあった「**作らない: 実力値・ランキング／即投了の連戦を抑える仕組み**」は
> **撤回された**。実力値・段級位・同一ペアの連戦の減衰は実装ずみで、**正本は `docs/online-rating-spec.md`**。
> ランキング（順位表）だけは引き続き作らない。
...
1. **FIFO**。先に並んだ2人を組む。実力値が無いので待ち時間の公平性だけを見る。
...
- 中身: NFKC正規化 → 辞書語を「leet 同値クラス＋文字伸ばし許容」の正規表現にして照合（`bitch` が `b1tch`/`biiitch` を拾う）
  → 許容リストで誤爆回避（`classmate` の `ass` はOK等）→ 該当部分を `＊` に伏せ字。
  ⚠️ **入力側を leet 変換する方式は 2026-08-28 にやめた**（§5.5）。
...
### 5.5 崩し字の吸収は「辞書側」でやる 🟢（2026-08-28 修正）

- **やめた方式**: 入力を崩し字変換（`4`→`a`）してから辞書と突き合わせる。辞書の `4ne`（＝死ね）まで `ane` に化けるため、
  `Kaneko` `Akane` `Yamane` `Jane` `Diane` `Anne` `Suzanne` `Vanessa` などが軒並み伏せ字になっていた。
- **今の方式**: 入力は素のまま。**辞書の各文字を正規表現の文字クラスへ展開**して照合する（`charPattern` / `CHAR_CLASS`）。
  - 辞書の**英字**はクラスへ（`u`→`[uv]`、`a`→`[a4]`）。`fvck` `b1tch` は従来どおり検知する
  - 辞書の**数字**はその文字そのものを要求する。`4ne` は `4ne`/`shi4ne` にだけ当たり、`ane` には当たらない
- **`i` と `l` は別クラスにする**（`i`→`[i1]`、`l`→`[l1]`）。`1` はどちらにも化けるが、`i` と `l` を互いに
  同一視すると `lose`＝`iose`、`anal`＝`anai` になり誤爆だけが増える（実測で英単語 −109件、ハンドル −242件、検知漏れ 0）。
- **`t`→`7` はクラスに入れない**。`shi`＋`7` が `shit` に化けて `hayashi7` `takahashi7` `kobayashi7` `ishii7` を
  巻き込む（`hayashi`＋0〜9999 の13%）。拾えるのは `b17ch` のように `t` を `7` で書いた綴りだけで割に合わない
  （`b1tch` のように `t` をそのまま書いた形は従来どおり検知する）。
- **照合用ビュー（`raw`）は英字と数字をすべて残す**。崩し字に使う数字だけを残して他を落とすと区切り扱いになり、
  `shin37` が `shin3`（＝`shine`）、`shin23` が `shin3` としてつながって誤爆する。
  区切り記号への対策は英字だけの `plain` ビューが担っているので、`raw` 側で落とす必要はない。
- **記号（`@ $ !`）もクラスに入れない**。`normalizeDisplayName` が NFKC → 半角英数字と `_ - .` 以外を除去、の順で
  整えてから伏せ字化するため、`f@ck` `$hit` はフィルタに到達しない。テストも同じ理由で
  「NFKC後に許可外の文字を含む入力」を検知対象から外している（`STRIPPED_BEFORE_FILTER`）。
- **ヒットの半分以上が英字でなければ伏せ字にしない**（`collectSubstringHits`）。`455`→`ass`、`884`→`bba`、
  `b84`→`bba`、`8008`→`boob` のように数字だけでも辞書語に化けるため、`hana55` `sora455` `yuki884` `bob84`
  のような普通のハンドルを巻き込む。数字で綴った形は読み手にも元の語には見えないので、伏せる価値より害が大きい。
  **代償**: `a55` 単体は通す。`a55hole` は辞書に `asshole`（`ass` と重複するが7文字で当たる）を
  入れて拾っている。
- 辞書語は照合前に小文字化する（`wordToPattern`）。大文字で登録されていた `BBA` が一度もマッチしていなかったため。
  上の半分ルールがあるので `bba` は部分一致のままでよく、`bbayaro` `xbba` も検知する。
  巻き込む実在語（`ABBA` `Abbas` `Sabbath` `Bubba` `Kobba` `cribbage`）は許容リストで除外する。
- `lose` は `Klose` を文字どおり含むので **`loser` に変更**した（許容リストの `close` も `closer` に差し替え）。
- 既知の残り誤爆: `anna1` `ana1` … `anal` の `l` を `1` と読む。`1`＝`l` は正当な崩し字なので構造的に避けにくい。
...
- **ON/OFFトグルは設定モーダル（`#settings-modal`）に置く**（2026-08-13 決定。ロビーには出さない）。
  見出しは「だれかと対戦で相手がいないとき」／チェック文言は「コンピュータと対局する」
  （2026-08-20 変更。以前は1文で「相手が見つからないときは自動でコンピュータと対局する（推奨）」。
  「（推奨）」は既定ONで伝わるので外した）。
  localStorage `shogi_bot_fallback`（既定 `true`・未設定はONとして扱う）。
  トグルの読み書きは **shogi.js**（全ページで読み込む）が持つので、`/online/` 以外のページでも
  設定を変えられる。`online-match.js` はこのキーを読むだけ。
