...
- **秒読みの読み上げ**（`shogi.js` の「秒読みの読み上げ」節）は端末の読み上げ機能を使い、日本語の音声が無い端末では短いビープに落とします。1手○秒は「残り30秒」「残り20秒」、切れ負けは「残り1分」「残り30秒」を予告し、どちらも最後の10秒を10・9…1と読みます（1手10秒だけは毎手喋り続けないよう5から）。🔴 **読み上げに渡す文字列は普通の表記にすること。**かなで書くと読み上げ機能が単語の切れ目を判断できず、「のこりいっぷん」が「のこりいぷん」のように崩れます（実測で確認）。🔴 **読む数字は画面の残り時間と必ず同じにすること。**記録係の「1・2・3…」は経過秒を数え上げる読み方で、あれは対局者に時計が見えていないから成り立ちます。残り時間が出ているこの画面でやると声と表示が食い違い、実際に戸惑いが出ました。🔴 **読むのは自分の手番だけ**です（相手の秒読みまで聞こえると自分が急かされているように感じるため。盤の危険エフェクトと同じ方針）。🔴 読んだところまでの記録は手番では戻さず、**残り時間が増えたときだけ**戻します（手番ごとに戻すと、切れ負けで手番が回るたび「残り30秒」を読み直します）。
