...
- 演出は `#shogi-board` ではなく **`#board-stage`** に足します（`renderBoard()` が毎手 `#shogi-board` を空にするため）。位置は `offsetLeft` と `clientWidth` から計算するので、盤を180度回していても演出ごと一緒に回って合います。文字だけは打ち消し回転が要ります（`.waza-fuda-card` / `.waza-cutin-flip`）。当たりの線は警告線（`.threat-overlay`）と同じ 1マス=40 の抽象座標ですが、**色は金**です（赤は「玉が取られる」警告が使っています）。
- 🔴 **札は盤の1辺を横切って隠します。**長く出さないこと。残す長さは技の格で決め（小技2秒・囲い3秒・大技4秒、はじめて出した技だけ+1秒）、**盤に触れたら消します**（capture で見るだけにして、指し手のタップは飲み込みません）。名前は棋譜バーに出したまま残るので、盤の札を短くしても情報は消えません。
