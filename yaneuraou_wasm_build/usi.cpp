...
void usi_cmdexec(Position& pos, StateListPtr& states, string& cmd)
{
...

	{
		...
		// この局面での合法な王手をすべて出力
			...
			for (auto m : MoveList<CHECKS_ALL>(pos))
				if (pos.legal(m))
					cout << m.move << ' ';
			cout << endl;
		}
...
EMSCRIPTEN_KEEPALIVE extern "C" int usi_command(const char *c_cmd) {
	...
	// Multi-thread mode: wait for all threads to be started
	for (Thread* th : Threads) {
		if (!th->threadStarted)
			return 1;
	}
#endif
	// In single-thread mode, skip thread check since there's no separate thread
