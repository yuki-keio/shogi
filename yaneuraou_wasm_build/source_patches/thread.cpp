...
// Single-thread mode: do NOT create std::thread, run synchronously
Thread::Thread(size_t n) : idx(n), exit(false), searching(false)
{
	// In single-thread mode, mark thread as immediately started/ready
	threadStarted = true;
}
#else
...
#endif // EMSCRIPTEN_SINGLE_THREAD
...
{
#if defined(EMSCRIPTEN_SINGLE_THREAD)
	// Single-thread mode: execute search synchronously
	searching = true;
	search();
	searching = false;
#else
...
#endif
	// In single-thread mode, search is always complete when start_searching returns
}
